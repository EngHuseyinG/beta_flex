...
class gunluk_testler extends StatefulWidget {
   const gunluk_testler({super.key});



  @override
  State<gunluk_testler> createState() => _gunluk_testlerState();
}




class _gunluk_testlerState extends State<gunluk_testler> {
...
late bool _circularprogress = false;
  ...
  Future _openandcloseDrawer() async {
    ...
    if(now.day < 10) {_day1 = '0${now.day.toString()}';}
    else{_day1 = '${now.day.toString()}';}

    var snapshot ;
    if(_tekrarTestChecked == true) {
      snapshot = await dbrefb.child(_day1).get();
    }
    else if(_tekrarTestChecked == false ) {
    ...
    if(snapshot.exists) {
      ...
      setState(() {
        ...
        if(_cubukluChecked == true && _cubuksuzChecked == false) {
          ...
          element['day'].toString().startsWith(now.day.toString())
              && element['month'].toString().startsWith(now.month.toString())
              && element['year'].toString().startsWith(now.year.toString())
              && element['D145'].toString().startsWith('1'));
          list1.addAll(i);
          _testcounter = i.length;
        }
...
// Save the document.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      key: _scaffoldKey,
      appBar: AppBar(
        title: Text('Günün Testleri',style: TextStyle(fontSize: 15, color: Colors.white),maxLines: 1 ,),
        centerTitle: true,
      ...
      body: Center(
        child: Column(
          ...
          children: [
                          ...
                          veriyok == false && _testcounter == 0 && _searchController.text.isEmpty ? Center(child: Column(children: [SizedBox(height: 5,),Text('Yükleniyor...'),CircularProgressIndicator(),],)) :
                          veriyok == false && _testcounter == 0 && _searchController.text.isNotEmpty ?  const Center(child: AutoSizeText("VERİ BULUNAMADI", style: TextStyle(fontSize: 25, color: Colors.black),maxLines: 1,),) :
                          veriyok == true && _testcounter == 0 ? const Center(child: AutoSizeText("Bugün için veri bulunamadı...", style: TextStyle(fontSize: 18, color: Colors.black),maxLines: 1,),) :
        Expanded(
          child:
            RawScrollbar(
              thickness: 20.0,
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      CheckboxListTile(
                        tristate: false,
                        title: Text('Çubuklu',style: TextStyle(fontSize: 12,color: Colors.black),),
                      ...
                      CheckboxListTile(
                        tristate: false,
                        title: Text('Çubuksuz',style: TextStyle(fontSize: 12,color: Colors.black),),
                        controlAffinity: ListTileControlAffinity.leading,
                      ...
                      CheckboxListTile(
                        tristate: false,
                        title: Text('Tekrar Testleri',style: TextStyle(fontSize: 12,color: Colors.black),),
                        controlAffinity: ListTileControlAffinity.leading,
